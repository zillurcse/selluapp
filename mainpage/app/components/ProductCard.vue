    ...
    <div class="relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-gray-50 border border-gray-100 transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-gray-200 group-hover:-translate-y-1">
      ...
      <div class="absolute top-4 w-full px-4 z-10 flex justify-between items-start pointer-events-none">
        <div class="flex flex-col gap-2">
          <span v-if="product.id % 3 === 0" class="px-3 py-1 bg-gray-900 text-white text-[10px] font-bold uppercase tracking-wider rounded-full shadow-md">New</span>
          <span v-if="product.id % 4 === 0" class="px-3 py-1 bg-rose-500 text-white text-[10px] font-bold uppercase tracking-wider rounded-full shadow-md">Sale</span>
        </div>
        <button 
          @click.stop="handleWishlist"
          class="p-2 rounded-full backdrop-blur-md bg-white/50 border border-white/40 shadow-sm hover:shadow-md transition-all pointer-events-auto"
      ...
      <div class="absolute inset-0 bg-gradient-to-t from-gray-900/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6 pointer-events-none">
        <button 
          class="w-full py-3 bg-white text-gray-900 rounded-full font-bold text-xs uppercase tracking-widest shadow-xl transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 hover:bg-gray-900 hover:text-white flex items-center justify-center gap-2 pointer-events-auto"
