  ...
  <div class="min-h-[60vh] flex flex-col">
    ...
    <template v-else-if="order">
      <AccountOrderTracking
        :order="order"
        @back="navigateTo(`/account/orders/${order.id}`)"
      />
    </template>
    ...
    <div v-else class="flex-1 flex flex-col items-center justify-center py-20 text-center px-4">
      ...
      <h2 class="text-xl font-black text-gray-900 mb-2">Tracking Not Found</h2>
      <p class="text-gray-500 max-w-xs mx-auto mb-8">We couldn't retrieve the tracking information for this order. Please wait a moment and try again.</p>
...
<script setup>
import AccountOrderTracking from '~/components/account/orders/AccountOrderTracking.vue'
