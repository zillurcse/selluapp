    ...
    <div class="max-w-7xl mx-auto">
      <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit Role: <span class="text-indigo-600 dark:text-indigo-400 capitalize">{{ form.name }}</span></h1>
          <p class="text-slate-500 dark:text-slate-400 mt-1">Update role details and customize permissions for this specific role.</p>
        </div>
      ...
      <div v-if="loadingInitial" class="flex flex-col items-center justify-center py-40">
        <div class="animate-spin rounded-full h-16 w-16 border-b-2 border-indigo-600 mb-4"></div>
        <p class="text-slate-500 dark:text-slate-400 font-bold animate-pulse">Loading role details...</p>
      </div>

      <form v-else @submit.prevent="updateRole" class="space-y-8 pb-20">
        <!-- Role Info Card -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 md:p-8 transition-all">
          <div class="max-w-2xl">
            <label for="name" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Role Name</label>
            <div class="relative group">
              <input
                ...
                class="block w-full px-4 py-3 rounded-xl border-slate-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all sm:text-sm placeholder-slate-400 dark:placeholder-slate-500 disabled:bg-slate-50 dark:disabled:bg-slate-950 disabled:cursor-not-allowed"
              />
            ...
            <p v-if="isSystemRole" class="mt-2 text-xs text-amber-600 dark:text-amber-400 italic flex items-center gap-1">
...
const { getItem, getAll, createItem, getById } = useCrud()
...
const isSystemRole = computed(() => {
  return ['super-admin', 'vendor'].includes(form.value.name)
})
...
const allSelected = computed(() => {
  ...
  return totalPerms > 0 && form.value.selectedPermissions.length === totalPerms
})
...
const fetchData = async () => {
    try {
        const [roleRes, permsRes] = await Promise.all([
            getById('/vendor/roles', roleId),
            getAll('/vendor/permissions')
        ])
        
        if (roleRes) {
            form.value.name = roleRes.name
            form.value.selectedPermissions = roleRes.permissions.map(p => p.name)
        }
        
        if (permsRes) {
            // Sort groups alphabetically
            const sorted = {}
            Object.keys(permsRes).sort().forEach(key => {
                sorted[key] = permsRes[key]
            })
...
const isGroupSelected = (perms) => {
  return perms.every(p => form.value.selectedPermissions.includes(p.name))
}
...
const toggleGroup = (group, perms) => {
  ...
  if (allInGroupSelected) {
    ...
    form.value.selectedPermissions = form.value.selectedPermissions.filter(p => !permNames.includes(p))
  } else {
    // Add all permissions of this group (avoid duplicates)
    const currentPerms = new Set(form.value.selectedPermissions)
    perms.forEach(p => currentPerms.add(p.name))
    form.value.selectedPermissions = Array.from(currentPerms)
  }
}
...
const toggleAllPermissions = () => {
  if (allSelected.value) {
    ...
    form.value.selectedPermissions = allNames
  }
}
...
const updateRole = async () => {
  try {
    ...
    await createItem('/vendor/roles', {
        name: form.value.name,
        permissions: form.value.selectedPermissions
    }, roleId)
