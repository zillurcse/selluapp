  ...
  <div class="p-10 bg-[#f8fafc] dark:bg-slate-950 min-h-screen transition-colors duration-300">
    ...
    <div class="max-w-[1400px] mx-auto mb-8">
      <div class="flex items-center gap-2 text-sm text-slate-500 font-medium mb-4">
        ...
        <ChevronRight class="w-4 h-4" />
        <span class="text-slate-900 dark:text-white font-bold">States / Provinces</span>
      </div>
      
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">States / Provinces</h1>
          <p class="text-slate-500 dark:text-slate-400 font-semibold mt-1">Manage states and regions for targeted shipping rules.</p>
        </div>
    ...
    <div class="max-w-[1400px] mx-auto mb-6 flex flex-col md:flex-row gap-4">
      ...
      <div class="w-full md:w-64">
        <select 
          ...
          @change="fetchStates"
          class="w-full h-12 px-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium text-slate-700 dark:text-slate-200 shadow-sm appearance-none cursor-pointer"
        >
    ...
    <!-- Table -->
    <div class="max-w-[1400px] mx-auto bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden text-center">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
              <th class="py-5 px-8 font-black text-[10px] uppercase tracking-wider text-slate-400">#</th>
              <th class="py-5 px-8 font-black text-[10px] uppercase tracking-wider text-slate-400">State Name</th>
              <th class="py-5 px-8 font-black text-[10px] uppercase tracking-wider text-slate-400">Country</th>
          ...
          <tbody>
            ...
            <tr v-else-if="states.length === 0">
              <td colspan="5" class="py-20 px-8 text-center text-slate-500 font-medium font-bold italic">No states found.</td>
            </tr>
            <tr v-for="(item, index) in states" :key="item.id" class="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
              <td class="py-5 px-8">
              ...
              <td class="py-5 px-8 text-indigo-500 font-bold italic">
                {{ item.country?.name || '—' }}
              </td>
              <td class="py-5 px-8">
                <span :class="[
                  ...
                  <span class="w-1.5 h-1.5 rounded-full" :class="item.status ? 'bg-emerald-600' : 'bg-slate-400'"></span>
                  {{ item.status ? 'Active' : 'Inactive' }}
                </span>
              </td>
              <td class="py-5 px-8 text-right">
                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button @click="editState(item)" class="p-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-indigo-600 rounded-lg transition-all">
      ...
      <div v-if="pagination.last_page > 1" class="p-6 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
        <p class="text-sm font-medium text-slate-500 font-bold italic">Showing {{ pagination.from }} to {{ pagination.to }} of {{ pagination.total }} states</p>
        <div class="flex gap-2">
          <button @click="changePage(pagination.current_page - 1)" :disabled="pagination.current_page === 1" class="p-2 rounded-xl border border-slate-200 dark:border-slate-800 disabled:opacity-50 hover:bg-slate-50">
            <ChevronLeft class="w-4 h-4" />
          </button>
          <button @click="changePage(pagination.current_page + 1)" :disabled="pagination.current_page === pagination.last_page" class="p-2 rounded-xl border border-slate-200 dark:border-slate-800 disabled:opacity-50 hover:bg-slate-50">
            <ChevronRight class="w-4 h-4" />
          </button>
        </div>
      </div>
        ...
        <div class="fixed inset-0 overflow-y-auto">
          <div class="flex min-h-full items-center justify-center p-4">
            <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-[32px] bg-white dark:bg-slate-900 p-8 shadow-2xl transition-all border border-slate-100 dark:border-slate-800">
              ...
              <form @submit.prevent="saveState" class="space-y-6">
                  <div class="space-y-2">
                    <AppSearchSelect 
                ...
                <div class="space-y-2">
                  <label class="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">State Name *</label>
                  <input 
                    v-model="form.name"
                    type="text" 
                    placeholder="e.g. California / Dhaka" 
                    required
                ...
                <div class="flex items-center gap-3 py-2">
                  <button 
                    ...
                    :class="['w-12 h-6 rounded-full transition-all duration-300 relative', form.status ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-700']"
                  >
                ...
                <div class="pt-4">
                  <button 
                    type="submit" 
                    :disabled="saving"
                    class="w-full h-12 bg-slate-900 hover:bg-black text-white font-black rounded-xl transition-all shadow-lg active:scale-95 disabled:opacity-50 shadow-slate-900/10"
                  >
                    {{ saving ? 'Saving...' : (editingId ? 'Update State' : 'Create State') }}
                  </button>
                </div>
              </form>
            </DialogPanel>
    ...
    <AppConfirmationModal
      ...
      title="Delete State"
      message="Are you sure you want to delete this state? This will also remove associated cities."
...
const fetchStates = async () => {
  try {
    pending.value = true
    const response = await getAll('/vendor/states', {
...
const fetchCountries = async () => {
  try {
    const response = await getAll('/vendor/all-countries')
    if (response.data) {
...
const saveState = async () => {
  try {
    saving.value = true
    if (editingId.value) {
      await updateItem(`/vendor/states/${editingId.value}`, form)
    } else {
      await createItem('/vendor/states', form, null, false)
    }
    closeModal()
    fetchStates()
  } catch (error) {
...
const confirmDelete = async () => {
  try {
    ...
    await deleteItem(itemToDelete.value, '/vendor/states')
    closeDeleteModal()
    fetchStates()
  } catch (error) {
    console.error(error)
