  ...
  <div class="page-wrapper">
    <div class="form-container">
      <div class="page-header">
        <button class="back-btn" @click="$router.push('/vendor/attributes/categories')"><ChevronLeft :size="16" /></button>
        <div>
          <h1 class="page-title">Edit Category</h1>
          <p class="page-subtitle">Modify existing category details and settings.</p>
        </div>
      </div>
            ...
            <div class="form-grid">
              <div class="field">
                ...
                <div class="upload-box upload-box--square" @click="openMediaLibrary('icon')">
                  ...
                  <div v-else class="upload-placeholder"><Upload :size="18" class="upload-icon" /><span>Choose Icon</span></div>
                  <div v-if="iconPreview" class="upload-change-overlay"><ImageIcon :size="16" /><span>Change</span></div>
                </div>
              </div>
              <div class="field">
                ...
                <div class="upload-box upload-box--wide" @click="openMediaLibrary('banner')">
                  ...
                  <div v-else class="upload-placeholder"><ImageIcon :size="18" class="upload-icon" /><span>Choose Banner</span></div>
      ...
      <div v-else class="loading-state">
        <Loader2 :size="24" class="spin" style="color:#4f46e5" />
...
definePageMeta({ middleware: 'auth', permissions: 'categories.edit' })

const route = useRoute(); const auth = useAuthStore(); const router = useRouter()
const config = useRuntimeConfig(); const { getItem, getAll, createItem } = useCrud()
const isLoading = ref(false); const form = ref(null); const categories = ref([])
const iconPreview = ref(null); const imagePreview = ref(null)

// Media Library
const showMediaModal = ref(false)
const mediaModalMode = ref('icon')
...
const handleMediaSelection = (selected) => {
  if (mediaModalMode.value === 'icon') {
    form.value.icon = selected.file_url
...
const fetchData = async () => {
  try {
    ...
    if (categoryRes?.data) {
      ...
      form.value = { ...cat, is_active: !!cat.is_active, icon: null, image: null }
      if (cat.icon) iconPreview.value = cat.icon
      if (cat.image) imagePreview.value = cat.image
    }
    const cats = categoriesRes?.data || categoriesRes || []
    categories.value = cats.filter(c => c.id != route.params.id)
...
const updateCategory = async () => {
  ...
  try {
    ...
    const utility = useUtilityStore(); utility.isEdit = true
    await createItem('/vendor/attributes/categories', fd, route.params.id)
