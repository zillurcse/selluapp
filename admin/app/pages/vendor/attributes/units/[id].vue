    ...
    <div class="form-container">
      <div class="page-header">
        ...
        <div>
          ...
          <p class="page-subtitle">Modify existing unit details and settings.</p>
        </div>
      </div>
...
definePageMeta({ middleware: 'auth', permissions: 'units.edit' })

const route = useRoute(); const router = useRouter(); const { getItem, updateItem } = useCrud()
const isLoading = ref(false); const form = ref(null)

const fetchData = async () => {
  try {
    const data = await getItem(`/vendor/attributes/units/${route.params.id}`)
    const unit = data?.data || data
    if (unit) form.value = { ...unit, is_active: !!unit.is_active }
...
const updateUnit = async () => {
  ...
  try {
    await updateItem(`/vendor/attributes/units/${route.params.id}`, { ...form.value, is_active: form.value.is_active ? 1 : 0 })
