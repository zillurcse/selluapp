  ...
  <div class="p-10 bg-[#f8fafc] dark:bg-slate-950 min-h-screen transition-colors duration-300">
    ...
    <div class="max-w-4xl mx-auto mb-8">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">Edit Unit</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 font-semibold opacity-80">Modify existing unit details and settings.</p>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto" v-if="form">
      <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.06)] dark:shadow-none border border-slate-200/60 dark:border-slate-800 p-8 md:p-12 transition-all">
        <form @submit.prevent="updateUnit" class="space-y-10">
          <!-- Basic Information -->
          <div class="space-y-6">
    ...
    <div v-else class="flex flex-col items-center justify-center py-20">
       ...
       <p class="text-slate-500 font-bold">Loading unit data...</p>
    </div>
  </div>
</template>
...
const updateUnit = async () => {
    ...
    try {
        // The API expects 1 or 0 for is_active, but the form uses boolean
        const payload = {
            ...form.value,
            is_active: form.value.is_active ? 1 : 0
        }
        await updateItem(`/vendor/attributes/units/${route.params.id}`, payload)
