    ...
    <div class="form-container">
      <div class="page-header">
        ...
        <div>
          ...
          <p class="page-subtitle">Modify existing brand details and settings.</p>
        </div>
      </div>
...
definePageMeta({ middleware: 'auth', permissions: 'brands.edit' })

const route = useRoute(); const auth = useAuthStore(); const router = useRouter()
const config = useRuntimeConfig(); const { getItem, updateItem } = useCrud()
const isLoading = ref(false); const form = ref(null); const imagePreview = ref(null)
...
const fetchData = async () => {
  try {
    ...
    if (brand) {
      form.value = { ...brand, is_active: !!brand.is_active, image: null }
      if (brand.image) imagePreview.value = brand.image
    }
...
const updateBrand = async () => {
  ...
  try {
    ...
    await updateItem(`/vendor/attributes/brands/${route.params.id}`, fd)
