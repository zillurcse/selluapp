  ...
  <div class="p-10 bg-[#f8fafc] dark:bg-slate-950 min-h-screen transition-colors duration-300">
    ...
    <div class="max-w-4xl mx-auto mb-8">
      <div class="flex items-center gap-4">
        <button @click="$router.push('/vendor/attributes/brands')" class="p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm active:scale-95 group">
          <ChevronLeft class="w-5 h-5 text-slate-600 dark:text-slate-300 group-hover:-translate-x-0.5 transition-transform" />
        </button>
        <div>
          <h1 class="text-2xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">Edit Brand</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 font-semibold opacity-80">Modify existing brand details and settings.</p>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto" v-if="form">
      <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.06)] dark:shadow-none border border-slate-200/60 dark:border-slate-800 p-8 md:p-12 transition-all">
        <form @submit.prevent="updateBrand" class="space-y-10">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="space-y-2">
                <label class="block text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Brand Name <span class="text-rose-500">*</span></label>
                <input v-model="form.name" type="text" placeholder="e.g. Apple, Samsung" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium" required />
              </div>
              <div class="space-y-2">
                <label class="block text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Sort Order</label>
                <input v-model="form.sort_order" type="number" placeholder="0" class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium" />
              </div>
            </div>

            <div class="space-y-2">
              <label class="block text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Description</label>
              <textarea v-model="form.description" rows="3" placeholder="Brief brand description..." class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium resize-none"></textarea>
            </div>
          </div>
          ...
          <div class="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3 mb-2">
              ...
              <h3 class="text-lg font-black text-slate-900 dark:text-white">Brand Assets</h3>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <!-- Logo Upload -->
              <div class="space-y-3">
                <label class="block text-sm font-black text-slate-700 dark:text-slate-300 ml-1">Brand Logo <span class="text-[10px] text-slate-400 uppercase tracking-widest ml-2">Square recommended</span></label>
                <div class="relative group cursor-pointer aspect-square max-w-[160px]">
                  ...
                  <div class="h-full border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl flex flex-col items-center justify-center group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50 transition-all overflow-hidden bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div v-else class="text-center p-4">
                      ...
                      <span class="text-[10px] font-black uppercase tracking-wider text-slate-400">Upload Logo</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800">
             <div class="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h4 class="text-sm font-black text-slate-900 dark:text-white leading-none">Brand Visibility</h4>
                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">Determine if this brand is active and displayed.</p>
                  </div>
                </div>
                <!-- Simple Toggle -->
          ...
          <div class="flex items-center justify-end gap-4 pt-10 mt-10 border-t border-slate-100 dark:border-slate-800">
             <button type="button" @click="$router.push('/vendor/attributes/brands')" class="px-8 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 font-black text-sm transition-all active:scale-95">
               Cancel
             </button>
    ...
    <div v-else class="flex flex-col items-center justify-center py-20">
       ...
       <p class="text-slate-500 font-bold">Loading brand data...</p>
    </div>
  </div>
</template>
...
const { getItem, updateItem } = useCrud()
...
const fetchData = async () => {
    try {
        const data = await getItem(`/vendor/attributes/brands/${route.params.id}`)
        if (data && (data.data || data)) {
            ...
            if (brand.image) imagePreview.value = `${config.public.apiBase}/storage/${brand.image}`
        }
    } catch (e) {
...
const updateBrand = async () => {
    ...
    try {
        ...
        await updateItem(`/vendor/attributes/brands/${route.params.id}`, formData)
